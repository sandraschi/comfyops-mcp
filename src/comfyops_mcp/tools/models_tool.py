        ...
        if operation == "health":
            ...
            if not health["ok"]:
                return {"success": False, "error": health["error"],
                        ...
                        "suggestions": ["Start ComfyUI first.",
                                        "Check COMFYOPS_COMFYUI_PORT and HOST."]}
            devices = health.get("cuda_devices", [])
            vram_total = (health.get("vram_total", 0) / (1024 ** 3)
                          if health.get("vram_total") else 0)
            vram_free = (health.get("vram_free", 0) / (1024 ** 3)
                         if health.get("vram_free") else 0)
            return {"success": True,
                    "comfyui_version": health.get("comfyui_version", "unknown"),
                    "cuda_devices": len(devices),
                    "vram_total_gb": round(vram_total, 1),
                    "vram_free_gb": round(vram_free, 1),
                    "message": (f"ComfyUI {health.get('comfyui_version', '?')} — "
